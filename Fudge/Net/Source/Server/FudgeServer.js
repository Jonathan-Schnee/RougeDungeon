...
export class FudgeServer {
    ...
    addEventListeners = () => {
        this.socket.on("connection", (_socket) => {
            ...
            try {
                ...
                this.logClients();
                let netMessage = { idTarget: id, command: FudgeNet.COMMAND.ASSIGN_ID };
    ...
    addUserOnValidLoginRequest(_wsConnection, _message) {
        ...
        for (let id in this.clients) {
            if (this.clients[id].name == name) {
                ...
                let netMessage = { idTarget: id, command: FudgeNet.COMMAND.LOGIN_RESPONSE, content: { success: false } };
                this.dispatch(netMessage);
                return;
            }
        }
        try {
            for (let id in this.clients) {
                ...
                if (client.socket == _wsConnection) {
                    client.name = name;
                    let netMessage = { idTarget: id, command: FudgeNet.COMMAND.ASSIGN_ID, content: { success: true } };
                    this.dispatch(netMessage);
                    return;
                }
            }
    ...
    heartbeat = () => {
        ...
        let message = { command: FudgeNet.COMMAND.SERVER_HEARTBEAT, content: clients };
        this.broadcast(message);
    };
}
